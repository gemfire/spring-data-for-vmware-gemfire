 ...
 * Description:
 * 2026-03-13: Created GemFire 10.0 RebalanceResults adapter
 */
...
/**
 * GUD API adapter for GemFire 10.0 RebalanceResults.
 */
public class GemFire100RebalanceResults implements GudRebalanceResults, NativeWrapper<RebalanceResults> {

    private final RebalanceResults nativeResults;

    public GemFire100RebalanceResults(RebalanceResults nativeResults) {
    ...
    public Set<GudPartitionRebalanceInfo> getPartitionRebalanceDetails() {
        return nativeResults.getPartitionRebalanceDetails().stream()
            .map(GemFire100PartitionRebalanceInfo::new)
