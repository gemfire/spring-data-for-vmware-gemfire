 ...
 * Description:
 * 2026-03-13: Created GemFire 10.0 CqQuery adapter
 */
...
/**
 * GUD API adapter for GemFire 10.0 CqQuery.
 */
public class GemFire100CqQuery implements GudCqQuery, NativeWrapper<CqQuery> {

    private final CqQuery nativeCq;

    public GemFire100CqQuery(CqQuery nativeCq) {
    ...
    public GudQuery getQuery() {
        return new GemFire100Query(nativeCq.getQuery());
    }

    @Override
    public GudCqAttributes getCqAttributes() {
        return new GemFire100CqAttributes(nativeCq.getCqAttributes());
    }

    @Override
    public GudCqAttributesMutator getCqAttributesMutator() {
        return new GemFire100CqAttributesMutator(nativeCq.getCqAttributesMutator());
    }

    @Override
    public GudCqStatistics getStatistics() {
        return new GemFire100CqStatistics(nativeCq.getStatistics());
    }
    ...
    public GudCqResults<Object> executeWithInitialResults() throws GudCqException {
        try {
            ...
            return new GemFire100CqResults<>(results);
        } catch (CqException | RegionNotFoundException e) {
            throw new GudCqException(e.getMessage(), e);
        }
    }
