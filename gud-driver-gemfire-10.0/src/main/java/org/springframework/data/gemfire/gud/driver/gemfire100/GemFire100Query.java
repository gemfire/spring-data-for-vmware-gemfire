 ...
 * Description:
 * 2026-03-13: Created GemFire 10.0 Query adapter
 */
...
/**
 * GUD API adapter for GemFire 10.0 Query.
 */
public class GemFire100Query implements GudQuery, NativeWrapper<Query> {

    private final Query nativeQuery;

    public GemFire100Query(Query nativeQuery) {
    ...
    public GudQueryStatistics getStatistics() {
        return new GemFire100QueryStatistics(nativeQuery.getStatistics());
    }
    ...
    private Object wrapResult(Object result) {
        if (result instanceof org.apache.geode.cache.query.SelectResults) {
            return new GemFire100SelectResults<>((org.apache.geode.cache.query.SelectResults<?>) result);
        }
        return result;
    }
}
