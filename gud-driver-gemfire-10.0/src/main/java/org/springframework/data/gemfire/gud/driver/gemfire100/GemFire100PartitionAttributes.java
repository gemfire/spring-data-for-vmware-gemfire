 ...
 * Description:
 * 2026-03-13: Created GemFire 10.0 PartitionAttributes adapter
 */
...
/**
 * GUD API adapter for GemFire 10.0 PartitionAttributes.
 *
 * @param <K> the key type
 * @param <V> the value type
 */
public class GemFire100PartitionAttributes<K, V> implements GudPartitionAttributes<K, V>, NativeWrapper<PartitionAttributes<K, V>> {

    private final PartitionAttributes<K, V> nativeAttributes;

    public GemFire100PartitionAttributes(PartitionAttributes<K, V> nativeAttributes) {
    ...
    public GudPartitionResolver<K, V> getPartitionResolver() {
        ...
        return resolver != null ? new GemFire100PartitionResolverWrapper<>(resolver) : null;
    }
    ...
    public List<GudPartitionListener> getPartitionListeners() {
        ...
        return java.util.Arrays.stream(listeners)
            .map(GemFire100PartitionListenerWrapper::new)
    ...
    public GudFixedPartitionAttributes[] getFixedPartitionAttributes() {
        ...
        return list.stream()
            .map(GemFire100FixedPartitionAttributes::new)
