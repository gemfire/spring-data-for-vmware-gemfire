 ...
 * Description:
 * 2026-03-13: Created GemFire 10.0 ClientRegionFactory adapter
 */
...
/**
 * GUD API adapter for GemFire 10.0 ClientRegionFactory.
 *
 * @param <K> the key type
 * @param <V> the value type
 */
public class GemFire100ClientRegionFactory<K, V> implements GudClientRegionFactory<K, V>, NativeWrapper<ClientRegionFactory<K, V>> {

    private final ClientRegionFactory<K, V> nativeFactory;

    public GemFire100ClientRegionFactory(ClientRegionFactory<K, V> nativeFactory) {
    ...
    public GudClientRegionFactory<K, V> setCacheLoader(GudCacheLoader<K, V> cacheLoader) {
        // ClientRegionFactory doesn't have setCacheLoader in 10.0 - use attributes
        return this;
    }
    ...
    public GudClientRegionFactory<K, V> setCacheWriter(GudCacheWriter<K, V> cacheWriter) {
        // ClientRegionFactory doesn't have setCacheWriter in 10.0 - use attributes
        return this;
    }
    ...
    public GudClientRegionFactory<K, V> addCacheListener(GudCacheListener<K, V> cacheListener) {
        if (cacheListener != null) {
            ...
            nativeFactory.addCacheListener(new GemFire100CacheListenerAdapter<>(cacheListener));
        }
        return this;
    }
        ...
        if (cacheListeners != null) {
            ...
            for (int i = 0; i < cacheListeners.length; i++) {
                if (cacheListeners[i] != null) {
                    nativeListeners[i] = new GemFire100CacheListenerAdapter<>(cacheListeners[i]);
                }
            }
    ...
    public GudRegion<K, V> create(String regionName) {
        return new GemFire100Region<>(nativeFactory.create(regionName));
    }
    ...
    public GudRegion<K, V> createSubregion(GudRegion<?, ?> parent, String subregionName) {
        if (parent instanceof NativeWrapper) {
            ...
            return new GemFire100Region<>(nativeFactory.createSubregion(nativeParent, subregionName));
        }
