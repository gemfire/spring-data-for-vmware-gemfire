 ...
 * Description:
 * 2026-03-11: Created GemFire 10.0 ClientCache adapter
 */
...
/**
 * GUD API adapter for GemFire 10.0 ClientCache.
 */
public class GemFire100ClientCache implements GudClientCache, NativeWrapper<ClientCache> {

    private final ClientCache nativeClientCache;

    public GemFire100ClientCache(ClientCache nativeClientCache) {
    ...
    public GudDistributedSystem getDistributedSystem() {
        return new GemFire100DistributedSystem(nativeClientCache.getDistributedSystem());
    }

    @Override
    public GudPool getDefaultPool() {
        return new GemFire100Pool(nativeClientCache.getDefaultPool());
    }
    ...
    public <K, V> GudClientRegionFactory<K, V> createClientRegionFactory(GudClientRegionShortcut shortcut) {
        ...
        return new GemFire100ClientRegionFactory<>(nativeClientCache.createClientRegionFactory(nativeShortcut));
    }
    ...
    public GudDiskStoreFactory createDiskStoreFactory() {
        return new GemFire100DiskStoreFactory(nativeClientCache.createDiskStoreFactory());
    }
    ...
    public GudDiskStore findDiskStore(String name) {
        ...
        return diskStore != null ? new GemFire100DiskStore(diskStore) : null;
    }
    ...
    public <K, V> GudRegion<K, V> getRegion(String path) {
        ...
        return region != null ? new GemFire100Region<>(region) : null;
    }
    ...
    public Set<GudRegion<?, ?>> rootRegions() {
        ...
        for (org.apache.geode.cache.Region<?, ?> region : nativeRegions) {
            result.add(new GemFire100Region<>(region));
        }
        return result;
    }
    ...
    public GudResourceManager getResourceManager() {
        return new GemFire100ResourceManager(nativeClientCache.getResourceManager());
    }
    ...
    public GudQueryService getLocalQueryService() {
        return new GemFire100QueryService(nativeClientCache.getLocalQueryService());
    }
