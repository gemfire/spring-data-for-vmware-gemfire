 ...
 * Description:
 * 2026-03-14: Created GemFire 10.0 driver implementation based on 10.3 driver
 */
...
/**
 * GUD driver implementation for GemFire 10.0.x.
 * Wraps native GemFire 10.0 types with GUD API interfaces.
 */
public class GemFire100Driver implements GudDriver {

    public static final String DRIVER_NAME = "gemfire-10.0";
    public static final String SUPPORTED_VERSION = "10.0";
    ...
    public GudClientCacheFactory createClientCacheFactory() {
        return new GemFire100ClientCacheFactory();
    }

    @Override
    public GudCacheFactory createCacheFactory() {
        return new GemFire100CacheFactory();
    }

    @Override
    public GudCache wrapCache(Object nativeCache) {
        if (nativeCache instanceof Cache) {
            return new GemFire100Cache((Cache) nativeCache);
        }
    ...
    public GudClientCache wrapClientCache(Object nativeClientCache) {
        if (nativeClientCache instanceof ClientCache) {
            return new GemFire100ClientCache((ClientCache) nativeClientCache);
        }
    ...
    public <K, V> GudRegion<K, V> wrapRegion(Object nativeRegion) {
        if (nativeRegion instanceof Region) {
            return new GemFire100Region<>((Region<K, V>) nativeRegion);
        }
    ...
    public GudPool wrapPool(Object nativePool) {
        if (nativePool instanceof Pool) {
            return new GemFire100Pool((Pool) nativePool);
        }
