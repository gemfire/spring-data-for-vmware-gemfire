 ...
 * Description:
 * 2026-03-11: Created GemFire 10.0 AttributesMutator adapter
 */
...
/**
 * GUD API adapter for GemFire 10.0 AttributesMutator.
 */
public class GemFire100AttributesMutator<K, V> implements GudAttributesMutator<K, V>, NativeWrapper<AttributesMutator<K, V>> {

    private final AttributesMutator<K, V> nativeMutator;

    public GemFire100AttributesMutator(AttributesMutator<K, V> nativeMutator) {
    ...
    public GudRegion<K, V> getRegion() {
        return new GemFire100Region<>(nativeMutator.getRegion());
    }
    ...
    public GudCacheLoader<K, V> setCacheLoader(GudCacheLoader<K, V> cacheLoader) {
        org.apache.geode.cache.CacheLoader<K, V> oldLoader = nativeMutator.setCacheLoader(
            cacheLoader != null ? new GemFire100CacheLoaderAdapter<>(cacheLoader) : null
        );
    ...
    public GudCacheWriter<K, V> setCacheWriter(GudCacheWriter<K, V> cacheWriter) {
        org.apache.geode.cache.CacheWriter<K, V> oldWriter = nativeMutator.setCacheWriter(
            cacheWriter != null ? new GemFire100CacheWriterAdapter<>(cacheWriter) : null
        );
    ...
    public void addCacheListener(GudCacheListener<K, V> cacheListener) {
        if (cacheListener != null) {
            nativeMutator.addCacheListener(new GemFire100CacheListenerAdapter<>(cacheListener));
        }
    }
    ...
    public GudEvictionAttributesMutator getEvictionAttributesMutator() {
        return new GemFire100EvictionAttributesMutator(nativeMutator.getEvictionAttributesMutator());
    }
