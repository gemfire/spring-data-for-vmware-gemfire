 ...
 * Description:
 * 2026-03-13: Created GemFire 10.0 EntryEvent wrapper
 */
...
/**
 * GemFire 10.0 implementation of GudEntryEvent.
 * Wraps the native EntryEvent and delegates all operations.
 *
 * @param <K> the key type
 * @param <V> the value type
 */
public class GemFire100EntryEvent<K, V> implements GudEntryEvent<K, V>, NativeWrapper<EntryEvent<K, V>> {

    private final EntryEvent<K, V> nativeEvent;

    public GemFire100EntryEvent(EntryEvent<K, V> nativeEvent) {
    ...
    public GudTransactionId getTransactionId() {
        return nativeEvent.getTransactionId() != null 
            ? new GemFire100TransactionId(nativeEvent.getTransactionId()) 
            : null;
    }

    @Override
    public GudRegion<K, V> getRegion() {
        return new GemFire100Region<>(nativeEvent.getRegion());
    }
    ...
    public GudDistributedMember getDistributedMember() {
        return new GemFire100DistributedMember(nativeEvent.getDistributedMember());
    }
