 ...
 */
public class GemFireCqQuery implements GudCqQuery, NativeWrapper<CqQuery> {

    private final CqQuery nativeCq;

    public GemFireCqQuery(CqQuery nativeCq) {
    ...
    public GudQuery getQuery() {
        return new GemFireQuery(nativeCq.getQuery());
    }

    @Override
    public GudCqAttributes getCqAttributes() {
        return new GemFireCqAttributes(nativeCq.getCqAttributes());
    }

    @Override
    public GudCqAttributesMutator getCqAttributesMutator() {
        return new GemFireCqAttributesMutator(nativeCq.getCqAttributesMutator());
    }

    @Override
    public GudCqStatistics getStatistics() {
        return new GemFireCqStatistics(nativeCq.getStatistics());
    }
    ...
    public GudCqResults<Object> executeWithInitialResults() throws GudCqException {
        try {
            ...
            return new GemFireCqResults<>(results);
        } catch (CqException | RegionNotFoundException e) {
            throw new GudCqException(e.getMessage(), e);
        }
    }
