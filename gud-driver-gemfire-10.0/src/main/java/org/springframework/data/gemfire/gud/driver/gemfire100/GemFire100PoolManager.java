 ...
 * Description:
 * 2026-03-13: Created GemFire 10.0 PoolManager implementation
 */
...
/**
 * GemFire 10.0 implementation of GudPoolManager.
 * Delegates to native PoolManager.
 */
public class GemFire100PoolManager implements GudPoolManager {

    @Override
    public GudPoolFactory createFactory() {
        return new GemFire100PoolFactory(PoolManager.createFactory());
    }

    @Override
    public GudPool find(String name) {
        Pool pool = PoolManager.find(name);
        return pool != null ? new GemFire100Pool(pool) : null;
    }
    ...
    public Map<String, GudPool> getAll() {
        ...
        for (Map.Entry<String, Pool> entry : PoolManager.getAll().entrySet()) {
            result.put(entry.getKey(), new GemFire100Pool(entry.getValue()));
        }
        return result;
    }
