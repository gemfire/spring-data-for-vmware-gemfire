 ...
 */
public class GemFire100CacheLoaderAdapter<K, V> implements CacheLoader<K, V> {

    private final GudCacheLoader<K, V> gudCacheLoader;

    public GemFire100CacheLoaderAdapter(GudCacheLoader<K, V> gudCacheLoader) {
    ...
    public V load(LoaderHelper<K, V> helper) throws CacheLoaderException {
        try {
            return gudCacheLoader.load(new GemFire100LoaderHelper<>(helper));
        } catch (Exception e) {
