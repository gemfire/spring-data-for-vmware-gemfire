...
public class EnableEntityDefinedRegionsUnitTests extends SpringApplicationContextIntegrationTestsSupport {
	...
	private <K, V> void assertRegion(Region<K, V> region, String name, Class<K> keyConstraint, Class<V> valueConstraint) {
	...
	private <K, V> void assertRegion(Region<K, V> region, String name, String fullPath, Class<K> keyConstraint,
			Class<V> valueConstraint) {
	...
	private <K, V> void assertRegionWithAttributes(Region<K, V> region, String name, DataPolicy dataPolicy,
		...
		assertThat(region.getAttributes()).isNotNull();
		assertRegionAttributes(region.getAttributes(), dataPolicy, diskStoreName, diskSynchronous, ignoreJta, poolName,
				scope);
	}
	...
	private <K, V> void assertPartitionAttributes(PartitionAttributes<K, V> partitionAttributes, String collocatedWith,
			PartitionResolver<?, ?> partitionResolver, Integer redundantCopies) {
	...
	private void assertFixedPartitionAttributes(FixedPartitionAttributes fixedPartitionAttributes, String partitionName,
			boolean primary, int numBuckets) {
	...
	private void assertUndefinedRegions(String... regionBeanNames) {
		...
		stream(nullSafeArray(regionBeanNames, String.class))
				.forEach(regionBeanName -> assertThat(containsBean(regionBeanName)).isFalse());

		assertThat(getBeansOfType(Region.class)).hasSize(9 - length(regionBeanNames));
	}
	...
	private FixedPartitionAttributes findFixedPartitionAttributes(PartitionAttributes<?, ?> partitionAttributes,
		...
		List<FixedPartitionAttributes> fixedPartitionAttributes = nullSafeList(
	...
	public void entityPeerPartitionRegionsDefined() {
		...
		assertRegionWithAttributes(customers, "Customers", DataPolicy.PERSISTENT_PARTITION, null, true, false, null,
				Scope.DISTRIBUTED_NO_ACK);
		assertPartitionAttributes(customers.getAttributes().getPartitionAttributes(), null, null, 1);
		assertFixedPartitionAttributes(
				findFixedPartitionAttributes(customers.getAttributes().getPartitionAttributes(), "one"), "one", true, 16);
		assertFixedPartitionAttributes(
				findFixedPartitionAttributes(customers.getAttributes().getPartitionAttributes(), "two"), "two", false, 21);

		Region<Object, Object> contactEvents = getBean("ContactEvents", Region.class);

		assertRegionWithAttributes(contactEvents, "ContactEvents", DataPolicy.PERSISTENT_PARTITION, "mockDiskStore", false,
				true, null, Scope.DISTRIBUTED_NO_ACK);
		...
		assertUndefinedRegions( "CollocatedPartitionRegionEntity", "GenericRegionEntity",
				"LocalRegionEntity", "NonEntity", "PartitionRegionEntity", "ReplicateRegionEntity", "Accounts");
	}
	...
	public void entityReplicateRegionAlreadyDefinedIgnoresEntityDefinedRegionDefinition() {
		...
		assertRegionWithAttributes(accounts, "Accounts", DataPolicy.REPLICATE, null, true, false, null,
				Scope.DISTRIBUTED_NO_ACK);
	}
	...
	public void entityServerRegionsDefined() {
		...
		assertRegionWithAttributes(localRegionEntity, "LocalRegionEntity", DataPolicy.NORMAL, null, true, false, null,
				Scope.LOCAL);

		Region<Object, Object> genericRegionEntity = getBean("GenericRegionEntity", Region.class);

		assertRegionWithAttributes(genericRegionEntity, "GenericRegionEntity", DataPolicy.PARTITION, null, true, false,
				null, Scope.DISTRIBUTED_NO_ACK);

		assertUndefinedRegions("CollocatedPartitionRegionEntity", "ContactEvents",
				"NonEntity", "PartitionRegionEntity", "ReplicateRegionEntity");
	}
...
//
//		assertRegionWithAttributes(accounts, "Sessions", DataPolicy.REPLICATE, null, true, false, null,
//				Scope.DISTRIBUTED_ACK);

		Region<Object, Object> genericRegionEntity = getBean("GenericRegionEntity", Region.class);

		assertRegionWithAttributes(genericRegionEntity, "GenericRegionEntity", DataPolicy.REPLICATE, null, true, false,
				null, Scope.DISTRIBUTED_ACK);
		...
		assertUndefinedRegions( "CollocatedPartitionRegionEntity", "ContactEvents", "NonEntity",
				"PartitionRegionEntity", "ReplicateRegionEntity", "Accounts");
	}
...
//
//		assertRegionWithAttributes(sessions, "Sessions", DataPolicy.PARTITION, null, true, false, null,
//				Scope.DISTRIBUTED_NO_ACK);
	...
	@EnableEntityDefinedRegions(basePackageClasses = NonEntity.class,
			excludeFilters = {
					@ComponentScan.Filter(type = FilterType.ANNOTATION, classes = { LocalRegion.class, ReplicateRegion.class }),
	...
	static class PeerPartitionRegionPersistentEntitiesConfiguration {
		...
		PartitionResolver<?, ?> mockPartitionResolver() {
			return mock(PartitionResolver.class, MockObjectsSupport.mockObjectIdentifier("MockPartitionResolver"));
		}
	}
	...
	@EnableEntityDefinedRegions(basePackageClasses = NonEntity.class, serverRegionShortcut = RegionShortcut.REPLICATE,
			excludeFilters = @ComponentScan.Filter(type = FilterType.ASSIGNABLE_TYPE,
					classes = { CollocatedPartitionRegionEntity.class, ReplicateRegionEntity.class }))
	static class ServerPersistentEntitiesWithCustomConfiguration {}

	@PeerCacheApplication
	@EnableGemFireMockObjects
	@EnableEntityDefinedRegions(basePackageClasses = NonEntity.class, excludeFilters = @ComponentScan.Filter(type = FilterType.ASSIGNABLE_TYPE,
					classes = { CollocatedPartitionRegionEntity.class, LocalRegionEntity.class, ReplicateRegionEntity.class }))
	...
	@EnableEntityDefinedRegions(basePackageClasses = NonEntity.class,
			excludeFilters = @ComponentScan.Filter(type = FilterType.ASSIGNABLE_TYPE,
					classes = { CollocatedPartitionRegionEntity.class, GenericRegionEntity.class, LocalRegionEntity.class,
	...
	@EnableEntityDefinedRegions(basePackageClasses = NonEntity.class,
			excludeFilters = @ComponentScan.Filter(type = FilterType.ASSIGNABLE_TYPE,
					classes = { CollocatedPartitionRegionEntity.class, GenericRegionEntity.class,
