...
public class EnableIndexingConfigurationUnitTests extends SpringApplicationContextIntegrationTestsSupport {
	...
	private void assertOqlIndexes(ConfigurableApplicationContext applicationContext) {
		...
		assertOqlIndex(customersFirstNameIndex, "CustomersFirstNameFunctionalIdx", "first_name", "/LoyalCustomers",
	...
	public void indexAnnotatedEntityPropertyIsIgnoredWithExistingIndexHavingSameDefinition() {
		...
		assertOqlIndex(firstNameIndex, "LoyalCustomersFirstNameFunctionalIdx", "first_name", "/LoyalCustomers",
				IndexType.FUNCTIONAL);
	...
	static class GemFireConfiguration {
		...
		Cache mockQueryService(Cache mockCache) throws Exception {
			...
			when(mockQueryService.createHashIndex(anyString(), anyString(), anyString())).thenAnswer(new HashIndexAnswer());

			when(mockQueryService.createIndex(anyString(), anyString(), anyString())).thenAnswer(new FunctionalIndexAnswer());

			when(mockQueryService.createKeyIndex(anyString(), anyString(), anyString())).thenAnswer(new KeyIndexAnswer());
			...
			for (Index index : indexes) {
				if (index.getIndexedExpression().equalsIgnoreCase(expression)
					...
					throw new IndexExistsException(
							String.format("Index [%1$s] has the same definition as existing Index [%2$s]", name, index.getName()));
	...
	@EnableEntityDefinedRegions(basePackageClasses = NonEntity.class,
			excludeFilters = @ComponentScan.Filter(type = FilterType.ASSIGNABLE_TYPE,
					value = { CollocatedPartitionRegionEntity.class, GenericRegionEntity.class, LocalRegionEntity.class,
