...
public class SubRegionRepositoryIntegrationTests extends IntegrationTestsSupport {
	...
	public void setup() {
		...
		Assertions.assertThat(programmers).as("The /Users/Programmers Subregion was null").isNotNull();

		if (programmers.isEmpty()) {
			programmers.putAll(PROGRAMMER_USER_DATA);
		}

		Assertions.assertThat(programmers.size()).isEqualTo(PROGRAMMER_USER_DATA.size());
		Assertions.assertThat(adminUsers).as("The /Local/Admins/Users Subregion was null").isNotNull();

		if (adminUsers.isEmpty()) {
			adminUsers.putAll(ADMIN_USER_DATA);
		}

		Assertions.assertThat(adminUsers.size()).isEqualTo(ADMIN_USER_DATA.size());
		Assertions.assertThat(guestUsers).as("The /Local/Guest/Users Subregion was null").isNotNull();

		if (guestUsers.isEmpty()) {
			guestUsers.putAll(GUEST_USER_DATA);
		}

		Assertions.assertThat(guestUsers.size()).isEqualTo(GUEST_USER_DATA.size());
	}

	@Test
	public void testSubregionRepositoryInteractions() {

		Assertions.assertThat(programmersRepo.findById("JamesGosling").orElse(null)).isEqualTo(PROGRAMMER_USER_DATA.get("JamesGosling"));

		List<Programmer> javaProgrammers = programmersRepo.findDistinctByProgrammingLanguageOrderByUsernameAsc("Java");

		Assertions.assertThat(javaProgrammers).isNotNull();
		Assertions.assertThat(javaProgrammers.isEmpty()).isFalse();
		Assertions.assertThat(javaProgrammers.size()).isEqualTo(2);
		Assertions.assertThat(getProgrammers("JamesGosling", "JoshuaBloch")).isEqualTo(javaProgrammers);

		List<Programmer> groovyProgrammers = programmersRepo.findDistinctByProgrammingLanguageLikeOrderByUsernameAsc("Groovy");

		Assertions.assertThat(groovyProgrammers).isNotNull();
		Assertions.assertThat(groovyProgrammers.isEmpty()).isFalse();
		Assertions.assertThat(groovyProgrammers.size()).isEqualTo(1);
		Assertions.assertThat(getProgrammers("JamesStrachan")).isEqualTo(groovyProgrammers);
		...
		Assertions.assertThat(javaProgrammers).isNotNull();
		Assertions.assertThat(javaProgrammers.isEmpty()).isFalse();
		Assertions.assertThat(javaProgrammers.size()).isEqualTo(3);
		Assertions.assertThat(getProgrammers("JamesGosling", "JoshuaBloch", "RodJohnson")).isEqualTo(javaProgrammers);

		groovyProgrammers = programmersRepo.findDistinctByProgrammingLanguageLikeOrderByUsernameAsc("Groovy");

		Assertions.assertThat(groovyProgrammers).isNotNull();
		Assertions.assertThat(groovyProgrammers.isEmpty()).isFalse();
		Assertions.assertThat(groovyProgrammers.size()).isEqualTo(3);
		Assertions.assertThat(getProgrammers("GraemeRocher", "GuillaumeLaforge", "JamesStrachan")).isEqualTo(groovyProgrammers);

		List<Programmer> javaLikeProgrammers = programmersRepo.findDistinctByProgrammingLanguageLikeOrderByUsernameAsc("Java%");

		Assertions.assertThat(javaLikeProgrammers).isNotNull();
		Assertions.assertThat(javaLikeProgrammers.isEmpty()).isFalse();
		Assertions.assertThat(javaLikeProgrammers.size()).isEqualTo(4);
		Assertions.assertThat(getProgrammers("BrendanEich", "JamesGosling", "JoshuaBloch", "RodJohnson"))
		...
		Assertions.assertThat(pseudoCodeProgrammers).isNotNull();
		Assertions.assertThat(pseudoCodeProgrammers.isEmpty()).isTrue();
	}

	@Test
	public void testIdenticallyNamedSubregionDataAccess() {

		Assertions.assertThat(adminUserRepo.findById("supertool").orElse(null)).isEqualTo(getAdminUser("supertool"));
		Assertions.assertThat(guestUserRepo.findById("joeblow").orElse(null)).isEqualTo(getGuestUser("joeblow"));

		List<RootUser> rootUsers = adminUserRepo.findDistinctByUsername("zeus");

		Assertions.assertThat(rootUsers).isNotNull();
		Assertions.assertThat(rootUsers.isEmpty()).isFalse();
		Assertions.assertThat(rootUsers.size()).isEqualTo(1);

		Assertions.assertThat(rootUsers.get(0)).isEqualTo(getAdminUser("zeus"));

		List<GuestUser> guestUsers = guestUserRepo.findDistinctByUsername("bubba");

		Assertions.assertThat(guestUsers).isNotNull();
		Assertions.assertThat(guestUsers.isEmpty()).isFalse();
		Assertions.assertThat(guestUsers.size()).isEqualTo(1);
		Assertions.assertThat(guestUsers.get(0)).isEqualTo(getGuestUser("bubba"));
	}
}
