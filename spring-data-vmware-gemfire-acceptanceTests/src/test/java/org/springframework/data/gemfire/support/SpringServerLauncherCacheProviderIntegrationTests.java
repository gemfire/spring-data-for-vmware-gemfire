...
public class SpringServerLauncherCacheProviderIntegrationTests extends IntegrationTestsSupport {
	...
	public void createCacheWithSpring() {
		...
		Assertions.assertThat(state.getStatus()).isEqualTo(Status.ONLINE);
		...
		Assertions.assertThat(cache.getName()).isEqualTo("TestMemberName");
		Assertions.assertThat(cache.getResourceManager().getCriticalHeapPercentage()).isEqualTo(55.0f);

		state = launcher.stop();

		Assertions.assertThat(state.getStatus()).isEqualTo(Status.STOPPED);
	}
}
