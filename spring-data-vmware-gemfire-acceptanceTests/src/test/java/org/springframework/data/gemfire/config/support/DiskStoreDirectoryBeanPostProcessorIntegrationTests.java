...
public class DiskStoreDirectoryBeanPostProcessorIntegrationTests extends IntegrationTestsSupport {
	...
	public static void testSuiteSetup() {
		Assertions.assertThat(new File("./gemfire/disk-stores/ds2/local").mkdirs()).isTrue();
	}

	@AfterClass
	public static void testSuiteTearDown() {
		Assertions.assertThat(FileSystemUtils.deleteRecursive(new File("./gemfire"))).isTrue();
		Assertions.assertThat(FileSystemUtils.deleteRecursive(new File("./gfe"))).isTrue();
	}

	@Test
	public void diskStoreDirectoriesExist() {
		Assertions.assertThat(new File("./gemfire/disk-stores/ds1").isDirectory()).isTrue();
		Assertions.assertThat(new File("./gemfire/disk-stores/ds2/local").isDirectory()).isTrue();
		Assertions.assertThat(new File("./gemfire/disk-stores/ds2/remote").isDirectory()).isTrue();
		Assertions.assertThat(new File("./gfe/ds/store3/local").isDirectory()).isTrue();
	}
