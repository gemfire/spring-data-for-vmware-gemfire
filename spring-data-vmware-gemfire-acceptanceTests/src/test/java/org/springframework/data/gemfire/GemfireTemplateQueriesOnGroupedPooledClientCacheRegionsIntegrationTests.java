...
public class GemfireTemplateQueriesOnGroupedPooledClientCacheRegionsIntegrationTests {
	...
	public void findsAllCats() {
		...
		Assertions.assertThat(catNames).isNotNull();
		Assertions.assertThat(catNames.size()).isEqualTo(5);
		Assertions.assertThat(catNames).containsAll(Arrays.asList("Grey", "Patchit", "Tyger", "Molly", "Sammy"));
	}
	...
	public void findsAllDogs() {
		...
		Assertions.assertThat(dogNames).isNotNull();
		Assertions.assertThat(dogNames.size()).isEqualTo(2);
		Assertions.assertThat(dogNames).containsAll(Arrays.asList("Spuds", "Maha"));
	}
