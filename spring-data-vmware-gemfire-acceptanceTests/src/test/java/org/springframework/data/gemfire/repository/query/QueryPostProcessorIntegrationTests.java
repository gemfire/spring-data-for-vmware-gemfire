...
public class QueryPostProcessorIntegrationTests extends IntegrationTestsSupport {
	...
	public void queryPostProcessingProcessesGeneratedOqlQueries() {
		...
		Assertions.assertThat(this.personRepository.count()).isEqualTo(11);
		Assertions.assertThat(this.userRepository.count()).isEqualTo(3);
		Assertions.assertThat(this.recordingQueryPostProcessor.queries).isEmpty();

		List<User> users = this.userRepository.findDistinctByUsernameLike("%doe");

		Assertions.assertThat(users).hasSize(1);
		Assertions.assertThat(this.recordingQueryPostProcessor.queries).hasSize(1);
		Assertions.assertThat(this.recordingQueryPostProcessor.queries)
		...
		Assertions.assertThat(bakingDoes).hasSize(3);
		Assertions.assertThat(this.recordingQueryPostProcessor.queries).hasSize(2);
		Assertions.assertThat(this.recordingQueryPostProcessor.queries).containsExactly(
		...
		Assertions.assertThat(jacks).hasSize(1);
		Assertions.assertThat(jacks.stream().findFirst().map(Person::getName).orElse(null)).isEqualTo("Jack Sparrow");

		Assertions.assertThat(this.recordingQueryPostProcessor.queries).hasSize(3);
		Assertions.assertThat(this.recordingQueryPostProcessor.queries).containsExactly(
			"SELECT DISTINCT * FROM /Users x WHERE x.username LIKE $1",
			"SELECT * FROM /simple x WHERE x.firstname IN SET ('Cookie', 'Pie', 'Sour')",
