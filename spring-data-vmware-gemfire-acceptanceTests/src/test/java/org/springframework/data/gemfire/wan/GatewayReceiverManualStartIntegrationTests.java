...
public class GatewayReceiverManualStartIntegrationTests extends IntegrationTestsSupport {
	...
	private void assertGreaterThanEqualToLessThanEqualTo(String message,
		...
		Assertions.assertThat(actualValue >= lowerBound && actualValue <= upperBound).as(message).isTrue();
	}

	@Test
	public void autoGatewayReceiverConfigurationIsCorrect() {

		Assertions.assertThat(autoGatewayReceiver)
			.describedAs("The 'Auto' GatewayReceiver was not properly configured or initialized")
			.isNotNull();

		Assertions.assertThat(autoGatewayReceiver.isRunning()).isTrue();
		Assertions.assertThat(autoGatewayReceiver.getStartPort()).isEqualTo(7070);
		Assertions.assertThat(autoGatewayReceiver.getEndPort()).isEqualTo(7700);
		...
		Assertions.assertThat(autoGatewayReceiver.isRunning()).isFalse();
	}

	@Test
	public void manualGatewayReceiverConfigurationIsCorrect() throws IOException {

		Assertions.assertThat(manualGatewayReceiver)
			.describedAs("The 'Manual' GatewayReceiver was not properly configured or initialized")
			.isNotNull();

		Assertions.assertThat(manualGatewayReceiver.isRunning()).isFalse();
		Assertions.assertThat(manualGatewayReceiver.getStartPort()).isEqualTo(6060);
		Assertions.assertThat(manualGatewayReceiver.getEndPort()).isEqualTo(6600);

		manualGatewayReceiver.start();

		Assertions.assertThat(manualGatewayReceiver.isRunning()).isTrue();
