...
public class FunctionExecutionIntegrationTests extends ForkingClientServerIntegrationTestsSupport {
	...
	public static void startGemFireServer() throws Exception {
		ForkingClientServerIntegrationTestsSupport.startGemFireServer(FunctionCacheServerProcess.class);
	}
	...
	public void setupGemFireClient() {
		...
		this.gemfireCache = new ClientCacheFactory()
			...
			.addPoolServer("localhost", Integer.getInteger(ClientServerIntegrationTestsSupport.GEMFIRE_CACHE_SERVER_PORT_PROPERTY))
			.create();

		Assertions.assertThat(this.gemfireCache).isNotNull();
		Assertions.assertThat(this.gemfireCache.getName()).isEqualTo(FunctionExecutionIntegrationTests.class.getSimpleName());

		this.gemfireRegion = this.gemfireCache.<String, String>createClientRegionFactory(ClientRegionShortcut.PROXY)
			.create("test-function");

		Assertions.assertThat(this.gemfireRegion).isNotNull();
		Assertions.assertThat(this.gemfireRegion.getName()).isEqualTo("test-function");

		this.gemfirePool = this.poolResolver.resolve("DEFAULT");

		Assertions.assertThat(this.gemfirePool).isNotNull();
		Assertions.assertThat(this.gemfirePool.getName()).isEqualTo("DEFAULT");
	}
	...
	private void verifyFunctionExecution(AbstractFunctionExecution functionExecution) {
		...
		for (String result : results) {
			Assertions.assertThat(result).isEqualTo(String.valueOf(count++));
		}
	}
}
