...
public class RepositoryQueryUsingInOperatorIntegrationTests extends IntegrationTestsSupport {
	...
	public void setup() {
		...
		Assertions.assertThat(this.userRepository.count()).isEqualTo(this.users.size());
	}
	...
	public void findByUserIdsInArrayIsCorrect() {
		...
		Assertions.assertThat(usersById).isNotNull();
		Assertions.assertThat(usersById).hasSize(2);
		Assertions.assertThat(usersById).containsExactly(findUsersByIds(2, 4));
	}
	...
	public void findByUserIdsInSetIsCorrect() {
		...
		Assertions.assertThat(usersById).isNotNull();
		Assertions.assertThat(usersById).hasSize(3);
		Assertions.assertThat(usersById).containsExactlyInAnyOrder(findUsersByIds(2, 3, 4));
	}
	...
	public void findByUserNamesInArrayIsCorrect() {
		...
		Assertions.assertThat(usersByName).isNotNull();
		Assertions.assertThat(usersByName).hasSize(3);
		Assertions.assertThat(usersByName).containsExactly(findUsersByNames("cookieDoe", "pieDoe", "sourDoe"));
	}
	...
	public void findByUserNamesInSetIsCorrect() {
		...
		Assertions.assertThat(usersByName).isNotNull();
		Assertions.assertThat(usersByName).hasSize(2);
		Assertions.assertThat(usersByName).containsExactly(findUsersByNames("jonDoe", "sourDoe"));
	}
