...
public class UserRepositoryQueriesIntegrationTests extends IntegrationTestsSupport {
	...
	private static void assertQueryResults(Iterable<User> actualUsers, String... expectedUsernames) {
		...
		Assertions.assertThat(actualUsers).as("The query did not return any results").isNotNull();
		...
		Assertions.assertThat(actualUsernames.size()).isEqualTo(expectedUsernames.length);
		Assertions.assertThat(actualUsernames.containsAll(Arrays.asList(expectedUsernames))).isTrue();
	}
	...
	public void setup() {
		...
		Assertions.assertThat(users).describedAs("The 'Users' GemFire Cache Region cannot be null").isNotNull();
		...
		if (users.isEmpty()) {
			...
			Assertions.assertThat(users.size()).isEqualTo(9);
		}
	}
	...
	public void testNonCollectionNonEntityResultQueries() {
		...
		Assertions.assertThat(toIntValue(count)).isEqualTo(3);

		count = userRepository.countUsersByUsernameLike("handy%");

		Assertions.assertThat(toIntValue(count)).isEqualTo(2);

		count = userRepository.countUsersByUsernameLike("smith%");

		Assertions.assertThat(count).isNotNull();
		Assertions.assertThat(toIntValue(count)).isEqualTo(0);
	}
}
