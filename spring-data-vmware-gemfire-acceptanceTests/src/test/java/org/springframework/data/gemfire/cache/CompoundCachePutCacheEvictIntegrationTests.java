...
public class CompoundCachePutCacheEvictIntegrationTests extends IntegrationTestsSupport {
	...
	private void assertPeopleInDepartment(Department department, Employee... people) {
		...
		Assertions.assertThat(peopleInDepartment).isNotNull();
		Assertions.assertThat(peopleInDepartment.size()).isEqualTo(people.length);
		Assertions.assertThat(peopleInDepartment).contains(people);
	}
	...
	public void setup() {
		...
		Assertions.assertThat(employeesRegion).containsValue(janeDoe);
		Assertions.assertThat(employeesRegion).containsValue(jonDoe);
	}
	...
	public void janeDoeUpdateSuccessful() {
		...
		Assertions.assertThat(employeeService.isCacheMiss()).isTrue();
		...
		Assertions.assertThat(employeesRegion).containsValue(janeDoe);
		assertPeopleInDepartment(Department.DESIGN, janeDoe);
		Assertions.assertThat(employeeService.isCacheMiss()).isFalse();
	}
