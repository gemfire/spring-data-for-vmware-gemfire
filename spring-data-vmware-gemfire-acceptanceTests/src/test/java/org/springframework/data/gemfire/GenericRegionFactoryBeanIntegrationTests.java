...
public class GenericRegionFactoryBeanIntegrationTests extends IntegrationTestsSupport {
	...
	public static void tearDown() {
		...
		FileFilter fileFilter = FileSystemUtils.CompositeFileFilter.or(
			FileSystemUtils.FileExtensionFilter.newFileExtensionFilter(".if"), FileSystemUtils.FileExtensionFilter.newFileExtensionFilter(".crf"), FileSystemUtils.FileExtensionFilter.newFileExtensionFilter(".drf"),
				FileSystemUtils.FileExtensionFilter.newFileExtensionFilter(".log"));
	...
	protected void assertRegionAttributes(Region<?, ?> region, String expectedRegionName, String expectedRegionPath,
		...
		Assertions.assertThat(region).as("The GemFire Cache Region must not be null").isNotNull();
		...
		Assertions.assertThat(region.getAttributes()).as("The RegionAttributes must be specified").isNotNull();
		Assertions.assertThat(region.getAttributes().getDataPolicy()).isEqualTo(expectedDataPolicy);
		Assertions.assertThat(region.getAttributes().getScope()).isEqualTo(expectedScope);
	}
