...
public class CommitConflictExceptionTransactionalIntegrationTests extends IntegrationTestsSupport {
	...
	public void cacheTransactionManagementIsConfigured() {
		...
		Assertions.assertThat(this.cache).isNotNull();
		Assertions.assertThat(this.cache.getName()).isEqualTo("CommitConflictExceptionTransactionalIntegrationTests");
		Assertions.assertThat(this.cache.getCopyOnRead()).isTrue();
		Assertions.assertThat(this.cache.getCacheTransactionManager()).isNotNull();
	}
	...
	static class TransactionalCommitConflictMultithreadedTestCase extends MultithreadedTestCase {
		...
		public void initialize() {
			...
			Assertions.assertThat(accountLoaded).isEqualTo(account);
		}
		...
		public void thread1() {
			...
			getAtm().process(1, account -> {
				...
				Assertions.assertThat(account.getNumber()).isEqualTo(1);
				Assertions.assertThat(account.getBalance()).isEqualTo(BigDecimal.valueOf(100.0d));
		...
		public void thread2() {
			...
			try {
				...
				org.junit.Assert.fail("Expected CommitConflictException");

			}
			catch (RuntimeException expected) {
				Assertions.assertThat(expected).isInstanceOf(GemfireTransactionCommitException.class);
				Assertions.assertThat(expected).hasCauseInstanceOf(CommitConflictException.class);
			}
		}
		...
		public void finish() {
			...
			Assertions.assertThat(account.getNumber()).isEqualTo(1);
			Assertions.assertThat(account.getBalance()).isEqualTo(BigDecimal.valueOf(50.0d));
		}
	}
	...
	public static class AutomatedTellerMachine {
		...
		public @NonNull Account findByAccountNumber(@NonNull Integer accountNumber) {
			...
			return getAccountRepository().findById(accountNumber)
				.orElseThrow(() -> RuntimeExceptionFactory.newIllegalStateException("Failed to find Account by number [%d]", accountNumber));
		}
