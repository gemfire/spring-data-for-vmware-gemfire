...
public class WiringDeclarableSupportIntegrationTests extends IntegrationTestsSupport {
	...
	private void assertRegion(Region<?, ?> region, String name, DataPolicy dataPolicy) {
		...
		Assertions.assertThat(region.getFullPath()).isEqualTo(String.format("%1$s%2$s", Region.SEPARATOR, name));
	...
	public void declarableObjectAutoWiredSuccessfully() {
		...
		Assertions.assertThat(beanFactory.containsBean("testBean")).isTrue();
		assertRegion(example, "Example", DataPolicy.NORMAL);

		CacheLoader<?, ?> testCacheLoader = example.getAttributes().getCacheLoader();

		Assertions.assertThat(testCacheLoader).isInstanceOf(TestCacheLoader.class);
		Assertions.assertThat(((TestCacheLoader) testCacheLoader).getBeanFactory()).isSameAs(beanFactory);
		Assertions.assertThat(((TestCacheLoader) testCacheLoader).getPropertyOne()).isEqualTo(beanFactory.getBean("testBean"));
		Assertions.assertThat(((TestCacheLoader) testCacheLoader).getPropertyTwo()).isEqualTo("GoodBye");
	}
