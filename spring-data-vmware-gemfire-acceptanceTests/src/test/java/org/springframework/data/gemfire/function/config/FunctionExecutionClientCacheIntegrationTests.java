...
public class FunctionExecutionClientCacheIntegrationTests extends IntegrationTestsSupport {
	...
	public void contextCreated() {
		...
		Assertions.assertThat(pool.getName()).isEqualTo("gemfirePool");
		Assertions.assertThat(cache.getDefaultPool().getLocators().isEmpty()).isTrue();
		Assertions.assertThat(cache.getDefaultPool().getServers().size()).isEqualTo(1);
		Assertions.assertThat(pool.getLocators().isEmpty()).isTrue();
		Assertions.assertThat(pool.getServers().size()).isEqualTo(1);
		Assertions.assertThat(cache.getDefaultPool().getServers().get(0)).isEqualTo(pool.getServers().get(0));

		Region<?, ?> region = requireApplicationContext().getBean("r1", Region.class);

		Assertions.assertThat(region.getName()).isEqualTo("r1");
		Assertions.assertThat(region.getAttributes()).isNotNull();
		Assertions.assertThat(region.getAttributes().getPoolName()).isNull();

		GemfireOnServerFunctionTemplate template = requireApplicationContext().getBean(GemfireOnServerFunctionTemplate.class);

		Assertions.assertThat(template.getResultCollector()).isInstanceOf(MyResultCollector.class);
	}
