...
public class SnapshotApplicationEventTriggeredImportsExportsIntegrationTests extends IntegrationTestsSupport {
	...
	public static void setupSnapshotDirectoryAndFiles() throws Exception {
		...
		Assertions.assertThat(snapshotsDirectory.isDirectory() || snapshotsDirectory.mkdirs()).isTrue();
		...
		Assertions.assertThat(peopleSnapshotFile.isFile() || peopleSnapshotFile.createNewFile()).isTrue();
		Assertions.assertThat(nonHandyNonDoeSnapshotFile.isFile() || nonHandyNonDoeSnapshotFile.createNewFile()).isTrue();
	}
	...
	private void assertPeople(Region<Long, Person> targetRegion, Person... people) {
		...
		Assertions.assertThat(targetRegion.size()).isEqualTo(people.length);

		Arrays.stream(ArrayUtils.nullSafeArray(people, Person.class))
