...
public class JSONRegionAdviceIntegrationTests extends IntegrationTestsSupport {
	...
	private static String toJson(Object bean) {
		...
		try {
			...
			throw RuntimeExceptionFactory.newIllegalArgumentException(cause, "Failed to convert object [%s] into JSON", bean);
		}
	}
	...
	public void putAndCreate() {
		...
		Assertions.assertThat(this.jsonRegion.put("keyOne", json)).isEqualTo(json);

		this.jsonRegion.create("keyTwo", json);

		Assertions.assertThat(this.jsonRegion.get("keyTwo")).isEqualTo(json);
	}
	...
	public void putAll() {
		...
		Assertions.assertThat(results.get("key1")).isEqualTo("{\"hello1\":\"world1\"}");
		Assertions.assertThat(results.get("key2")).isEqualTo("{\"hello2\":\"world2\"}");
	}
	...
	public void objectToJSon() {
		...
		Assertions.assertThat(toJson(davidTuranski)).isEqualTo(json);

		Object result = jsonRegion.put("dave", davidTuranski);

		Assertions.assertThat(result).isEqualTo(toJson(davidTuranski));
	}
	...
	public void templateFind() {
		...
		Assertions.assertThat(results.iterator().next()).isEqualTo(toJson(davidTuranski));
	}
	...
	public void templateFindUnique() {
		...
		Assertions.assertThat(json).isEqualTo(toJson(davidTuranski));
	}
