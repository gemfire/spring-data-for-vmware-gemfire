...
public class RepositoryDataAccessOnRegionUsingCacheLoaderIntegrationTests extends IntegrationTestsSupport {
	...
	public void setup() {
		...
		Assertions.assertThat(this.people.getName()).isEqualTo("simple");
		Assertions.assertThat(this.people).isEmpty();
	}
	...
	public void repositoryFindByIdInvokesRegionCacheLoader() {
		...
		Assertions.assertThat(jonDoe.getId()).isEqualTo(2L);
		Assertions.assertThat(jonDoe.getFirstname()).isEqualTo("Jon");
		Assertions.assertThat(jonDoe.getLastname()).isEqualTo("Doe");
	}
