...
public class ClientRegionWithCacheLoaderCacheWriterIntegrationTests extends IntegrationTestsSupport {
	...
	public void testCacheLoaderWriter() {
		...
		Assertions.assertThat(localAppData).isNotNull();
		Assertions.assertThat(localAppData.size()).isEqualTo(0);

		for (int key = 0; key < REGION_SIZE; key++) {
			Assertions.assertThat(localAppData.get(key).intValue()).isEqualTo(key + 1);
		}

		Assertions.assertThat(localAppData.size()).isEqualTo(REGION_SIZE);

		for (int key = 0; key < REGION_SIZE; key++) {
			Assertions.assertThat(localAppData.put(key, REGION_SIZE - key).intValue()).isEqualTo(key + 1);
		}

		LocalAppDataCacheWriter localCacheWriter = applicationContext
			.getBean("localCacheWriter", LocalAppDataCacheWriter.class);

		Assertions.assertThat(localCacheWriter).isNotNull();

		for (int key = 0; key < REGION_SIZE; key++) {
			Assertions.assertThat(localCacheWriter.get(key).intValue()).isEqualTo(REGION_SIZE - key);
		}
	}
