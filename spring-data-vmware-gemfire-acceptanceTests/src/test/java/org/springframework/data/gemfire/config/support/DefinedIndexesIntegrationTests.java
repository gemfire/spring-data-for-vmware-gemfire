...
public class DefinedIndexesIntegrationTests extends IntegrationTestsSupport {
	...
	public void setup() {
		...
		put(people, Person.newPerson("Jon", "Doe", Person.newBirthDate(1989, Month.NOVEMBER, 11), Gender.MALE));
		put(people, Person.newPerson("Jane", "Doe", Person.newBirthDate(1991, Month.APRIL, 4), Gender.FEMALE));
		put(people, Person.newPerson("Pie", "Doe", Person.newBirthDate(2008, Month.JUNE, 21), Gender.FEMALE));
		put(people, Person.newPerson("Cookie", "Doe", Person.newBirthDate(2008, Month.AUGUST, 14), Gender.FEMALE));
	}
	...
	public void indexesCreated() {
		...
		Assertions.assertThat(definedIndexNames).isEqualTo(expectedDefinedIndexNames);
		Assertions.assertThat(id).isEqualTo(queryService.getIndex(people, id.getName()));
		Assertions.assertThat(birthDate).isEqualTo(queryService.getIndex(people, birthDate.getName()));
		Assertions.assertThat(lastName).isEqualTo(queryService.getIndex(people, lastName.getName()));
		Assertions.assertThat(name).isEqualTo(queryService.getIndex(people, name.getName()));
	}
				...
				public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
					...
					if (bean instanceof Index) {
						if ("LastNameIdx".equals(beanName)) {
							Assertions.assertThat(CacheFactory.getAnyInstance().getQueryService().getIndexes().contains(bean)).isTrue();
							Assertions.assertThat(beanName).isEqualToIgnoringCase(((Index) bean).getName());
						}
						else {
