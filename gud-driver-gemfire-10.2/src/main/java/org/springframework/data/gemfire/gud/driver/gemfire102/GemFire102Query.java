 ...
 * Description:
 * 2026-03-13: Created GemFire 10.2 Query adapter
 */
...
/**
 * GUD API adapter for GemFire 10.2 Query.
 */
public class GemFire102Query implements GudQuery, NativeWrapper<Query> {

    private final Query nativeQuery;

    public GemFire102Query(Query nativeQuery) {
    ...
    public GudQueryStatistics getStatistics() {
        return new GemFire102QueryStatistics(nativeQuery.getStatistics());
    }
    ...
    private Object wrapResult(Object result) {
        if (result instanceof org.apache.geode.cache.query.SelectResults) {
            return new GemFire102SelectResults<>((org.apache.geode.cache.query.SelectResults<?>) result);
        }
        return result;
    }
}
