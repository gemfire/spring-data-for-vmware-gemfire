 ...
 */
public class GemFire102CacheLoaderAdapter<K, V> implements CacheLoader<K, V> {

    private final GudCacheLoader<K, V> gudCacheLoader;

    public GemFire102CacheLoaderAdapter(GudCacheLoader<K, V> gudCacheLoader) {
    ...
    public V load(LoaderHelper<K, V> helper) throws CacheLoaderException {
        try {
            return gudCacheLoader.load(new GemFire102LoaderHelper<>(helper));
        } catch (Exception e) {
