 ...
 */
public class GemFireRegion<K, V> implements GudRegion<K, V>, NativeWrapper<Region<K, V>> {
    ...
    public GudRegion<?, ?> getParentRegion() {
        ...
        return parent != null ? new GemFireRegion<>(parent) : null;
    }
    ...
    public <SK, SV> GudRegion<SK, SV> getSubregion(String path) {
        ...
        return subregion != null ? new GemFireRegion<>(subregion) : null;
    }

    @Override
    public Set<GudRegion<?, ?>> subregions(boolean recursive) {
        return nativeRegion.subregions(recursive).stream()
            .map(r -> (GudRegion<?, ?>) new GemFireRegion<>(r))
            .collect(Collectors.toSet());
    }

    @Override
    public GudRegionAttributes<K, V> getAttributes() {
        return new GemFireRegionAttributes<>(nativeRegion.getAttributes());
    }

    @Override
    public GudAttributesMutator<K, V> getAttributesMutator() {
        return new GemFireAttributesMutator<>(nativeRegion.getAttributesMutator());
    }
