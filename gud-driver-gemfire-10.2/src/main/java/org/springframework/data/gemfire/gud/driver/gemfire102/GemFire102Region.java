 ...
 * Description:
 * 2026-03-11: Created GemFire 10.2 Region adapter
 */
...
/**
 * GUD API adapter for GemFire 10.2 Region.
 *
 * @param <K> the key type
 * @param <V> the value type
 */
public class GemFire102Region<K, V> implements GudRegion<K, V>, NativeWrapper<Region<K, V>> {

    private final Region<K, V> nativeRegion;

    public GemFire102Region(Region<K, V> nativeRegion) {
    ...
    public GudRegion<?, ?> getParentRegion() {
        ...
        return parent != null ? new GemFire102Region<>(parent) : null;
    }
    ...
    public <SK, SV> GudRegion<SK, SV> getSubregion(String path) {
        ...
        return subregion != null ? new GemFire102Region<>(subregion) : null;
    }

    @Override
    public Set<GudRegion<?, ?>> subregions(boolean recursive) {
        return nativeRegion.subregions(recursive).stream()
            .map(r -> (GudRegion<?, ?>) new GemFire102Region<>(r))
            .collect(Collectors.toSet());
    }

    @Override
    public GudRegionAttributes<K, V> getAttributes() {
        return new GemFire102RegionAttributes<>(nativeRegion.getAttributes());
    }

    @Override
    public GudAttributesMutator<K, V> getAttributesMutator() {
        return new GemFire102AttributesMutator<>(nativeRegion.getAttributesMutator());
    }
