 ...
 */
public class GemFireClientCache implements GudClientCache, NativeWrapper<ClientCache> {

    private final ClientCache nativeClientCache;

    public GemFireClientCache(ClientCache nativeClientCache) {
    ...
    public GudDistributedSystem getDistributedSystem() {
        return new GemFireDistributedSystem(nativeClientCache.getDistributedSystem());
    }

    @Override
    public GudPool getDefaultPool() {
        return new GemFirePool(nativeClientCache.getDefaultPool());
    }
    ...
    public <K, V> GudClientRegionFactory<K, V> createClientRegionFactory(GudClientRegionShortcut shortcut) {
        ...
        return new GemFireClientRegionFactory<>(nativeClientCache.createClientRegionFactory(nativeShortcut));
    }
    ...
    public GudDiskStoreFactory createDiskStoreFactory() {
        return new GemFireDiskStoreFactory(nativeClientCache.createDiskStoreFactory());
    }
    ...
    public GudDiskStore findDiskStore(String name) {
        ...
        return diskStore != null ? new GemFireDiskStore(diskStore) : null;
    }
    ...
    public Set<GudRegion<?, ?>> rootRegions() {
        ...
        for (org.apache.geode.cache.Region<?, ?> region : nativeRegions) {
            result.add(new GemFireRegion<>(region));
        }
        return result;
    }
    ...
    public GudResourceManager getResourceManager() {
        return new GemFireResourceManager(nativeClientCache.getResourceManager());
    }
    ...
    public GudQueryService getLocalQueryService() {
        return new GemFireQueryService(nativeClientCache.getLocalQueryService());
    }
