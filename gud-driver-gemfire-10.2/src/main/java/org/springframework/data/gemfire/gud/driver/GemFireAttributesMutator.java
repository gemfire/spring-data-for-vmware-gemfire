 ...
 */
public class GemFireAttributesMutator<K, V> implements GudAttributesMutator<K, V>, NativeWrapper<AttributesMutator<K, V>> {

    private final AttributesMutator<K, V> nativeMutator;

    public GemFireAttributesMutator(AttributesMutator<K, V> nativeMutator) {
    ...
    public GudRegion<K, V> getRegion() {
        return new GemFireRegion<>(nativeMutator.getRegion());
    }
    ...
    public GudCacheLoader<K, V> setCacheLoader(GudCacheLoader<K, V> cacheLoader) {
        org.apache.geode.cache.CacheLoader<K, V> oldLoader = nativeMutator.setCacheLoader(
            cacheLoader != null ? new GemFireCacheLoaderAdapter<>(cacheLoader) : null
        );
    ...
    public GudCacheWriter<K, V> setCacheWriter(GudCacheWriter<K, V> cacheWriter) {
        org.apache.geode.cache.CacheWriter<K, V> oldWriter = nativeMutator.setCacheWriter(
            cacheWriter != null ? new GemFireCacheWriterAdapter<>(cacheWriter) : null
        );
    ...
    public void addCacheListener(GudCacheListener<K, V> cacheListener) {
        if (cacheListener != null) {
            nativeMutator.addCacheListener(new GemFireCacheListenerAdapter<>(cacheListener));
        }
    }
    ...
    public GudEvictionAttributesMutator getEvictionAttributesMutator() {
        return new GemFireEvictionAttributesMutator(nativeMutator.getEvictionAttributesMutator());
    }
