 ...
 * Description:
 * 2026-03-13: Created GemFire 10.2 PartitionAttributes adapter
 */
...
/**
 * GUD API adapter for GemFire 10.2 PartitionAttributes.
 *
 * @param <K> the key type
 * @param <V> the value type
 */
public class GemFire102PartitionAttributes<K, V> implements GudPartitionAttributes<K, V>, NativeWrapper<PartitionAttributes<K, V>> {

    private final PartitionAttributes<K, V> nativeAttributes;

    public GemFire102PartitionAttributes(PartitionAttributes<K, V> nativeAttributes) {
    ...
    public GudPartitionResolver<K, V> getPartitionResolver() {
        ...
        return resolver != null ? new GemFire102PartitionResolverWrapper<>(resolver) : null;
    }
    ...
    public List<GudPartitionListener> getPartitionListeners() {
        ...
        return java.util.Arrays.stream(listeners)
            .map(GemFire102PartitionListenerWrapper::new)
    ...
    public GudFixedPartitionAttributes[] getFixedPartitionAttributes() {
        ...
        return list.stream()
            .map(GemFire102FixedPartitionAttributes::new)
