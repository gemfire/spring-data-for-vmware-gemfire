 ...
 * Description:
 * 2026-03-14: Created GemFire 10.2 driver implementation based on 10.3 driver
 */
...
/**
 * GUD driver implementation for GemFire 10.2.x.
 * Wraps native GemFire 10.2 types with GUD API interfaces.
 */
public class GemFire102Driver implements GudDriver {

    public static final String DRIVER_NAME = "gemfire-10.2";
    public static final String SUPPORTED_VERSION = "10.2";
    ...
    public GudClientCacheFactory createClientCacheFactory() {
        return new GemFire102ClientCacheFactory();
    }

    @Override
    public GudCacheFactory createCacheFactory() {
        return new GemFire102CacheFactory();
    }

    @Override
    public GudCache wrapCache(Object nativeCache) {
        if (nativeCache instanceof Cache) {
            return new GemFire102Cache((Cache) nativeCache);
        }
    ...
    public GudClientCache wrapClientCache(Object nativeClientCache) {
        if (nativeClientCache instanceof ClientCache) {
            return new GemFire102ClientCache((ClientCache) nativeClientCache);
        }
    ...
    public <K, V> GudRegion<K, V> wrapRegion(Object nativeRegion) {
        if (nativeRegion instanceof Region) {
            return new GemFire102Region<>((Region<K, V>) nativeRegion);
        }
    ...
    public GudPool wrapPool(Object nativePool) {
        if (nativePool instanceof Pool) {
            return new GemFire102Pool((Pool) nativePool);
        }
