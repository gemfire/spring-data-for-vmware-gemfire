 ...
 * Description:
 * 2026-03-13: Created GemFire 10.1 EntryEvent wrapper
 */
...
/**
 * GemFire 10.1 implementation of GudEntryEvent.
 * Wraps the native EntryEvent and delegates all operations.
 *
 * @param <K> the key type
 * @param <V> the value type
 */
public class GemFire101EntryEvent<K, V> implements GudEntryEvent<K, V>, NativeWrapper<EntryEvent<K, V>> {

    private final EntryEvent<K, V> nativeEvent;

    public GemFire101EntryEvent(EntryEvent<K, V> nativeEvent) {
    ...
    public GudTransactionId getTransactionId() {
        return nativeEvent.getTransactionId() != null 
            ? new GemFire101TransactionId(nativeEvent.getTransactionId()) 
            : null;
    }

    @Override
    public GudRegion<K, V> getRegion() {
        return new GemFire101Region<>(nativeEvent.getRegion());
    }
    ...
    public GudDistributedMember getDistributedMember() {
        return new GemFire101DistributedMember(nativeEvent.getDistributedMember());
    }
