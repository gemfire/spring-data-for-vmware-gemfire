 ...
 */
public class GemFire101PoolManager implements GudPoolManager {

    @Override
    public GudPoolFactory createFactory() {
        return new GemFire101PoolFactory(PoolManager.createFactory());
    }

    @Override
    public GudPool find(String name) {
        Pool pool = PoolManager.find(name);
        return pool != null ? new GemFire101Pool(pool) : null;
    }
    ...
    public Map<String, GudPool> getAll() {
        ...
        for (Map.Entry<String, Pool> entry : PoolManager.getAll().entrySet()) {
            result.put(entry.getKey(), new GemFire101Pool(entry.getValue()));
        }
        return result;
    }
