...
/**
 * GUD driver implementation for GemFire 10.1.x.
 * Wraps native GemFire 10.1 types with GUD API interfaces.
 */
public class GemFire101Driver implements GudDriver {

    public static final String DRIVER_NAME = "gemfire-10.1";
    public static final String SUPPORTED_VERSION = "10.1";
    ...
    public GudClientCacheFactory createClientCacheFactory() {
        return new GemFire101ClientCacheFactory();
    }

    @Override
    public GudCacheFactory createCacheFactory() {
        return new GemFire101CacheFactory();
    }

    @Override
    public GudCache wrapCache(Object nativeCache) {
        if (nativeCache instanceof Cache) {
            return new GemFire101Cache((Cache) nativeCache);
        }
    ...
    public GudClientCache wrapClientCache(Object nativeClientCache) {
        if (nativeClientCache instanceof ClientCache) {
            return new GemFire101ClientCache((ClientCache) nativeClientCache);
        }
    ...
    public <K, V> GudRegion<K, V> wrapRegion(Object nativeRegion) {
        if (nativeRegion instanceof Region) {
            return new GemFire101Region<>((Region<K, V>) nativeRegion);
        }
    ...
    public GudPool wrapPool(Object nativePool) {
        if (nativePool instanceof Pool) {
            return new GemFire101Pool((Pool) nativePool);
        }
