 ...
 */
public class GemFire101CacheLoaderAdapter<K, V> implements CacheLoader<K, V> {

    private final GudCacheLoader<K, V> gudCacheLoader;

    public GemFire101CacheLoaderAdapter(GudCacheLoader<K, V> gudCacheLoader) {
    ...
    public V load(LoaderHelper<K, V> helper) throws CacheLoaderException {
        try {
            return gudCacheLoader.load(new GemFire101LoaderHelper<>(helper));
        } catch (Exception e) {
