 ...
 */
public class GemFireQuery implements GudQuery, NativeWrapper<Query> {

    private final Query nativeQuery;

    public GemFireQuery(Query nativeQuery) {
    ...
    public GudQueryStatistics getStatistics() {
        return new GemFireQueryStatistics(nativeQuery.getStatistics());
    }
    ...
    private Object wrapResult(Object result) {
        if (result instanceof org.apache.geode.cache.query.SelectResults) {
            return new GemFireSelectResults<>((org.apache.geode.cache.query.SelectResults<?>) result);
        }
        return result;
    }
}
