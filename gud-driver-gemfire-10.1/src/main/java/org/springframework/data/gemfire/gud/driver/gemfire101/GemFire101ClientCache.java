 ...
 * Description:
 * 2026-03-11: Created GemFire 10.1 ClientCache adapter
 */
...
/**
 * GUD API adapter for GemFire 10.1 ClientCache.
 */
public class GemFire101ClientCache implements GudClientCache, NativeWrapper<ClientCache> {

    private final ClientCache nativeClientCache;

    public GemFire101ClientCache(ClientCache nativeClientCache) {
    ...
    public GudDistributedSystem getDistributedSystem() {
        return new GemFire101DistributedSystem(nativeClientCache.getDistributedSystem());
    }

    @Override
    public GudPool getDefaultPool() {
        return new GemFire101Pool(nativeClientCache.getDefaultPool());
    }
    ...
    public <K, V> GudClientRegionFactory<K, V> createClientRegionFactory(GudClientRegionShortcut shortcut) {
        ...
        return new GemFire101ClientRegionFactory<>(nativeClientCache.createClientRegionFactory(nativeShortcut));
    }
    ...
    public GudDiskStoreFactory createDiskStoreFactory() {
        return new GemFire101DiskStoreFactory(nativeClientCache.createDiskStoreFactory());
    }
    ...
    public GudDiskStore findDiskStore(String name) {
        ...
        return diskStore != null ? new GemFire101DiskStore(diskStore) : null;
    }
    ...
    public <K, V> GudRegion<K, V> getRegion(String path) {
        ...
        return region != null ? new GemFire101Region<>(region) : null;
    }
    ...
    public Set<GudRegion<?, ?>> rootRegions() {
        ...
        for (org.apache.geode.cache.Region<?, ?> region : nativeRegions) {
            result.add(new GemFire101Region<>(region));
        }
        return result;
    }
    ...
    public GudResourceManager getResourceManager() {
        return new GemFire101ResourceManager(nativeClientCache.getResourceManager());
    }
    ...
    public GudQueryService getLocalQueryService() {
        return new GemFire101QueryService(nativeClientCache.getLocalQueryService());
    }
