 ...
 * Description:
 * 2026-03-13: Created GemFire 10.1 Query adapter
 */
...
/**
 * GUD API adapter for GemFire 10.1 Query.
 */
public class GemFire101Query implements GudQuery, NativeWrapper<Query> {

    private final Query nativeQuery;

    public GemFire101Query(Query nativeQuery) {
    ...
    public GudQueryStatistics getStatistics() {
        return new GemFire101QueryStatistics(nativeQuery.getStatistics());
    }
    ...
    private Object wrapResult(Object result) {
        if (result instanceof org.apache.geode.cache.query.SelectResults) {
            return new GemFire101SelectResults<>((org.apache.geode.cache.query.SelectResults<?>) result);
        }
        return result;
    }
}
