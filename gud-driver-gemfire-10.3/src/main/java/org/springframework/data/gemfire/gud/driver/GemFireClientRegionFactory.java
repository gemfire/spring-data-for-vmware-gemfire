 ...
 * Description:
 * 2026-03-13: Created GemFire 10.3 ClientRegionFactory adapter
 * 2026-03-31: Added setServerRegionName override (10.3 feature)
 */
...
/**
 * GUD API adapter for GemFire 10.3 ClientRegionFactory.
 *
 * @param <K> the key type
 * @param <V> the value type
 */
public class GemFireClientRegionFactory<K, V> implements GudClientRegionFactory<K, V>, NativeWrapper<ClientRegionFactory<K, V>> {

    private final ClientRegionFactory<K, V> nativeFactory;

    public GemFireClientRegionFactory(ClientRegionFactory<K, V> nativeFactory) {
    ...
    public GudClientRegionFactory<K, V> setCacheLoader(GudCacheLoader<K, V> cacheLoader) {
        // ClientRegionFactory doesn't have setCacheLoader in 10.3 - use attributes
        return this;
    }
    ...
    public GudClientRegionFactory<K, V> setCacheWriter(GudCacheWriter<K, V> cacheWriter) {
        // ClientRegionFactory doesn't have setCacheWriter in 10.3 - use attributes
        return this;
    }
    ...
    public GudClientRegionFactory<K, V> addCacheListener(GudCacheListener<K, V> cacheListener) {
        if (cacheListener != null) {
            ...
            nativeFactory.addCacheListener(new GemFireCacheListenerAdapter<>(cacheListener));
        }
        return this;
    }
        ...
        if (cacheListeners != null) {
            ...
            for (int i = 0; i < cacheListeners.length; i++) {
                if (cacheListeners[i] != null) {
                    nativeListeners[i] = new GemFireCacheListenerAdapter<>(cacheListeners[i]);
                }
            }
    ...
    @Override
    public GudClientRegionFactory<K, V> setServerRegionName(String serverRegionName) {
        nativeFactory.setServerRegionName(serverRegionName);
        return this;
    }
