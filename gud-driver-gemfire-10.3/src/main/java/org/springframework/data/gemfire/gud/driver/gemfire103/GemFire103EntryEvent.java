 ...
 * Description:
 * 2026-03-13: Created GemFire 10.3 EntryEvent wrapper
 */
...
/**
 * GemFire 10.3 implementation of GudEntryEvent.
 * Wraps the native EntryEvent and delegates all operations.
 *
 * @param <K> the key type
 * @param <V> the value type
 */
public class GemFire103EntryEvent<K, V> implements GudEntryEvent<K, V>, NativeWrapper<EntryEvent<K, V>> {

    private final EntryEvent<K, V> nativeEvent;

    public GemFire103EntryEvent(EntryEvent<K, V> nativeEvent) {
    ...
    public GudTransactionId getTransactionId() {
        return nativeEvent.getTransactionId() != null 
            ? new GemFire103TransactionId(nativeEvent.getTransactionId()) 
            : null;
    }

    @Override
    public GudRegion<K, V> getRegion() {
        return new GemFire103Region<>(nativeEvent.getRegion());
    }
    ...
    public GudDistributedMember getDistributedMember() {
        return new GemFire103DistributedMember(nativeEvent.getDistributedMember());
    }
