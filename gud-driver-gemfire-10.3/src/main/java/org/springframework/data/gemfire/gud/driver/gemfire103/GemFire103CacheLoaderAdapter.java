 ...
 */
public class GemFire103CacheLoaderAdapter<K, V> implements CacheLoader<K, V> {

    private final GudCacheLoader<K, V> gudCacheLoader;

    public GemFire103CacheLoaderAdapter(GudCacheLoader<K, V> gudCacheLoader) {
    ...
    public V load(LoaderHelper<K, V> helper) throws CacheLoaderException {
        try {
            return gudCacheLoader.load(new GemFire103LoaderHelper<>(helper));
        } catch (Exception e) {
