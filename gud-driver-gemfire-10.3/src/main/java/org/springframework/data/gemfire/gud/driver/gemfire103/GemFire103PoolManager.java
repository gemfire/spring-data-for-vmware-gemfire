 ...
 * Description:
 * 2026-03-13: Created GemFire 10.3 PoolManager implementation
 */
...
/**
 * GemFire 10.3 implementation of GudPoolManager.
 * Delegates to native PoolManager.
 */
public class GemFire103PoolManager implements GudPoolManager {

    @Override
    public GudPoolFactory createFactory() {
        return new GemFire103PoolFactory(PoolManager.createFactory());
    }

    @Override
    public GudPool find(String name) {
        Pool pool = PoolManager.find(name);
        return pool != null ? new GemFire103Pool(pool) : null;
    }
    ...
    public Map<String, GudPool> getAll() {
        ...
        for (Map.Entry<String, Pool> entry : PoolManager.getAll().entrySet()) {
            result.put(entry.getKey(), new GemFire103Pool(entry.getValue()));
        }
        return result;
    }
