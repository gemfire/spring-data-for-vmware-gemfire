 ...
 */
public class GemFire103CacheListenerAdapter<K, V> implements CacheListener<K, V>, NativeWrapper<GudCacheListener<K, V>> {

    private final GudCacheListener<K, V> gudListener;

    public GemFire103CacheListenerAdapter(GudCacheListener<K, V> gudListener) {
    ...
    public void afterCreate(EntryEvent<K, V> event) {
        gudListener.afterCreate(new GemFire103EntryEvent<>(event));
    }

    @Override
    public void afterUpdate(EntryEvent<K, V> event) {
        gudListener.afterUpdate(new GemFire103EntryEvent<>(event));
    }

    @Override
    public void afterInvalidate(EntryEvent<K, V> event) {
        gudListener.afterInvalidate(new GemFire103EntryEvent<>(event));
    }

    @Override
    public void afterDestroy(EntryEvent<K, V> event) {
        gudListener.afterDestroy(new GemFire103EntryEvent<>(event));
    }

    @Override
    public void afterRegionInvalidate(RegionEvent<K, V> event) {
        gudListener.afterRegionInvalidate(new GemFire103RegionEvent<>(event));
    }

    @Override
    public void afterRegionDestroy(RegionEvent<K, V> event) {
        gudListener.afterRegionDestroy(new GemFire103RegionEvent<>(event));
    }

    @Override
    public void afterRegionClear(RegionEvent<K, V> event) {
        gudListener.afterRegionClear(new GemFire103RegionEvent<>(event));
    }

    @Override
    public void afterRegionCreate(RegionEvent<K, V> event) {
        gudListener.afterRegionCreate(new GemFire103RegionEvent<>(event));
    }

    @Override
    public void afterRegionLive(RegionEvent<K, V> event) {
        gudListener.afterRegionLive(new GemFire103RegionEvent<>(event));
    }
