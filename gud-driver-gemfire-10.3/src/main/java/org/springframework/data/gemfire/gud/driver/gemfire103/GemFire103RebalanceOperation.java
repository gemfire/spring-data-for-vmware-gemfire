 ...
 */
public class GemFire103RebalanceOperation implements GudRebalanceOperation, NativeWrapper<RebalanceOperation> {

    private final RebalanceOperation nativeOperation;

    public GemFire103RebalanceOperation(RebalanceOperation nativeOperation) {
    ...
    public GudRebalanceResults getResults() throws CancellationException, InterruptedException {
        return new GemFire103RebalanceResults(nativeOperation.getResults());
    }

    @Override
    public GudRebalanceResults getResults(long timeout, TimeUnit unit) throws InterruptedException {
        try {
            return new GemFire103RebalanceResults(nativeOperation.getResults(timeout, unit));
