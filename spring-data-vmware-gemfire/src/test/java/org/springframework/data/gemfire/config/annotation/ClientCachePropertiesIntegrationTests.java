...
public class ClientCachePropertiesIntegrationTests extends SpringApplicationContextIntegrationTestsSupport {
	...
	@Test
	public void clientCacheConfigurationConnectionPerServer() {
		...
		MockPropertySource testPropertySource = new MockPropertySource()
				...
				.withProperty("spring.data.gemfire.pool.max-connections-per-server", 250)
		...
		newApplicationContext(testPropertySource, TestClientCacheConfigurationConnectionsPerServer.class);

		assertThat(containsBean("gemfireCache")).isTrue();
		assertThat(containsBean("mockPdxSerializer")).isTrue();

		ClientCacheFactoryBean testClientCacheFactoryBean = getBean("&gemfireCache", ClientCacheFactoryBean.class);
		...
		assertThat(defaultPool.getMaxConnectionsPerServer()).isEqualTo(250);
		assertThat(defaultPool.getMinConnectionsPerServer()).isEqualTo(50);
		...
		assertThat(defaultPool.getSubscriptionMessageTrackingTimeout())
				.isEqualTo(PoolFactory.DEFAULT_SUBSCRIPTION_MESSAGE_TRACKING_TIMEOUT);
	...
	@Test
	public void dynamicClientCacheConfigurationConnectionPerServer() {
		...
		MockPropertySource testPropertySource = new MockPropertySource()
				...
				.withProperty("spring.data.gemfire.pool.default.max-connections-per-server", 100)
				.withProperty("spring.data.gemfire.pool.default.min-connections-per-server", 10)
	...
	@EnableGemFireMockObjects
	@ClientCacheApplication(name = "TestClientCache", copyOnRead = true, criticalHeapPercentage = 95.0f,
			evictionHeapPercentage = 80.0f, idleTimeout = 15000L, maxConnectionsPerServer = 100, minConnectionsPerServer = 10,
			pingInterval = 15000L, readTimeout = 15000, retryAttempts = 1, socketConnectTimeout = 20001,
			subscriptionEnabled = true, subscriptionRedundancy = 1)
	@EnablePdx(ignoreUnreadFields = true, readSerialized = true, serializerBeanName = "mockPdxSerializer")
	@SuppressWarnings("unused")
	static class TestClientCacheConfigurationConnectionsPerServer {
		...
		ClientCacheConfigurer testClientCacheConfigurer() {
			...
			return (beanName, factoryBean) -> {
				...
				factoryBean.setPdxReadSerialized(false);
				factoryBean.setLoadConditioningInterval(180000);
				factoryBean.setMinConnectionsPerServer(50);
				factoryBean.setStatisticsInterval(500);
			};
		}
