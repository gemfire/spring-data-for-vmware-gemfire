...
public class PoolPropertiesIntegrationTests extends SpringApplicationContextIntegrationTestsSupport {
	...
	private void assertPool(Pool pool, int freeConnectionTimeout, long idleTimeout, int loadConditioningInterval,
			...
			boolean multiUserAuthentication, String name, long pingInterval, boolean prSinglehopEnabled, int readTimeout,
			int retryAttempts, int serverConnectionTimeout, String serverGroup, int socketBufferSize,
	...
	public void poolConfiguration() {
		...
		assertThat(testPool.getSubscriptionMessageTrackingTimeout())
	...
	public void multiPoolConfiguration() {
		...
		assertPool(defaultPool, 15000, 20000L, 180000, 275, 27, -1, 1, true, "DEFAULT", 15000L, false, 2000, 1, 60000,
				"testDefaultGroup", 16384, 10000, SocketFactory.DEFAULT, 500, 250, true, 300000, 3);

		Pool testPoolOne = getBean("TestPoolOne", Pool.class);

		assertPool(testPoolOne, 30000, 300000L, 120000, 500, 50, -1, 1, true, "TestPoolOne", 5000L, false, 15000, 2, 30000,
				"testGroup", 8192, 5000, mockSocketFactoryOne, 1000, 5000, true, 180000, 2);

		Pool testPoolTwo = getBean("TestPoolTwo", Pool.class);

		assertPool(testPoolTwo, 20000, 15000L, 60000, 1000, 100, -1, 1, true, "TestPoolTwo", 20000L, false, 5000, 4, 30000,
				"testTwoGroup", 65536, 15000, mockSocketFactoryTwo, 2000, 500, true, 300000, 4);
