...
public class CachingDefinedRegionsUsesCacheConfigCacheNamesIntegrationTests extends IntegrationTestsSupport {
	...
	public void assertClientCacheAndRegionConfiguration() {
		...
		Assertions.assertThat(this.clientCache.getName())
			.isEqualTo(CachingDefinedRegionsUsesCacheConfigCacheNamesIntegrationTests.class.getSimpleName());

		Assertions.assertThat(CollectionUtils.nullSafeSet(this.clientCache.rootRegions()).stream()
		...
		Arrays.asList(this.a, this.b, this.c, this.d).forEach(region -> {
			...
			Assertions.assertThat(region).isEmpty();
		});
	}
	...
	public void testNonCacheableMethodCachesNothing() {
		...
		Assertions.assertThat(this.service.nonCacheableMethod("0")).isEqualTo("TEST");

		Arrays.asList(this.a, this.b, this.c, this.d).forEach(region -> {
			Assertions.assertThat(region).doesNotContainKey("0");
			Assertions.assertThat(region).isEmpty();
		});
	}
	...
	public void cacheableMethodOneCachesToAAndB() {
		...
		Assertions.assertThat(this.service.cacheableMethodOne("2")).isEqualTo("ONE");

		Arrays.asList(this.a, this.b).forEach(region -> Assertions.assertThat(region).containsKey("2"));
		Arrays.asList(this.c, this.d).forEach(region -> Assertions.assertThat(region).isEmpty());
	}

	@Test
	public void cacheableMethodTwoCachesToCOnly() {

		Assertions.assertThat(this.service.cacheableMethodTwo("3")).isEqualTo("TWO");

		Collections.singletonList(this.c).forEach(region -> Assertions.assertThat(region).containsKey("3"));
		Arrays.asList(this.a, this.b, this.d).forEach(region -> Assertions.assertThat(region).isEmpty());
	}

	@Test
	public void cacheableMethodThreeCachesToAAndD() {

		Assertions.assertThat(this.service.cacheableMethodThree("4")).isEqualTo("THREE");

		Arrays.asList(this.a, this.d).forEach(region -> Assertions.assertThat(region).containsKey("4"));
		Arrays.asList(this.b, this.c).forEach(region -> Assertions.assertThat(region).isEmpty());
	}
