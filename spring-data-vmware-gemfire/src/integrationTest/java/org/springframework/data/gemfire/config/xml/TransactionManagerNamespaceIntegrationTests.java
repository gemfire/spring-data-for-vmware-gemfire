...
public class TransactionManagerNamespaceIntegrationTests extends IntegrationTestsSupport {
	...
	public void basicCacheWithTransactionsIsConfiguredCorrectly() {
		...
		Assertions.assertThat(requireApplicationContext().containsBean("gemfireTransactionManager")).isTrue();
		Assertions.assertThat(requireApplicationContext().containsBean("gemfire-transaction-manager")).isTrue();
		...
		Assertions.assertThat(transactionManager.isCopyOnRead()).isFalse();
	}
}
