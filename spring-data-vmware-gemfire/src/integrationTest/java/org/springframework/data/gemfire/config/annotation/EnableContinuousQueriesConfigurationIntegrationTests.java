...
public class EnableContinuousQueriesConfigurationIntegrationTests extends IntegrationTestsSupport {
	...
	public void setup() {
		...
		Assertions.assertThat(temperatureReading).isNotNull();
		Assertions.assertThat(temperatureReading.temperature()).isEqualTo(99);

		Assertions.assertThat(this.temperatureReadings.sizeOnServer()).isEqualTo(10);

		IntegrationTestsSupport.waitOn(() -> totalTemperatureReadingsCounter.get() >= 5, 100L);

		Assertions.assertThat(totalTemperatureReadingsCounter.get()).isNotZero();
	}

	@Test
	public void boilingTemperatureReadingsEqualsThree() {
		Assertions.assertThat(boilingTemperatureReadingsCounter.get()).isEqualTo(3);
	}

	@Test
	public void freezingTemperatureReadingsEqualsTwo() {
		Assertions.assertThat(freezingTemperatureReadingsCounter.get()).isEqualTo(2);
	}
	...
	static class TestConfiguration {
		...
		ClientRegionFactoryBean<Long, TemperatureReading> temperatureReadingsRegion(ClientCache gemfireCache) {
			...
			temperatureReadings.setCacheListeners(ArrayUtils.asArray(temperatureReadingCounterListener()));
