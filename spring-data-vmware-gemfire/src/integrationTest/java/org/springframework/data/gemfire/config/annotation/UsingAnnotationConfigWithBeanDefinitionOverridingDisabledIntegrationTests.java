...
public class UsingAnnotationConfigWithBeanDefinitionOverridingDisabledIntegrationTests extends IntegrationTestsSupport {
	...
	public void assertApplicationContextBeanDefinitionOverridingIsDisabled() {
		...
		Assertions.assertThat(Arrays.stream(applicationContext.getBeanDefinitionNames())
	...
	public void gemfireCacheSecurityAndSslConfigurationIsCorrect() {
		...
		Assertions.assertThat(this.cache.getName())
		...
		Assertions.assertThat(distributedSystem).isNotNull();

		Properties gemfireProperties = distributedSystem.getProperties();

		Assertions.assertThat(gemfireProperties).isNotNull();
		Assertions.assertThat(gemfireProperties).isNotEmpty();

		Assertions.assertThat(gemfireProperties.getProperty(GemFireProperties.SECURITY_MANAGER.getName()))
			.isEqualTo(String.valueOf(TestSecurityManager.class.getName()));

		Assertions.assertThat(gemfireProperties.getProperty(GemFireProperties.SSL_KEYSTORE.getName()))
			.isEqualTo("/path/to/test/keystore.jks");

		Assertions.assertThat(gemfireProperties.getProperty(GemFireProperties.SSL_KEYSTORE_PASSWORD.getName()))
			.isEqualTo("p@55w0rd");
	}
