...
public class RegionLookupIntegrationTests extends IntegrationTestsSupport {
	...
	public void withEnableRegionLookups() {
		...
		try {
			...
			Assertions.assertThat(applicationContext.containsBean("SpringLocalRegion")).isTrue();
			...
			Assertions.assertThat(nativeChildRegion.getAttributes().getDataPolicy()).isEqualTo(DataPolicy.NORMAL);

			Region<?, ?> springLocalRegion = applicationContext.getBean("SpringLocalRegion", Region.class);

			Assertions.assertThat(springLocalRegion).isNotNull();
			Assertions.assertThat(springLocalRegion.getName()).isEqualTo("SpringLocalRegion");
			Assertions.assertThat(springLocalRegion.getFullPath()).isEqualTo("/SpringLocalRegion");
			Assertions.assertThat(springLocalRegion.getAttributes()).isNotNull();
			Assertions.assertThat(springLocalRegion.getAttributes().getDataPolicy()).isEqualTo(DataPolicy.NORMAL);
		}
		finally {
			IntegrationTestsSupport.closeApplicationContext(applicationContext);
		}
