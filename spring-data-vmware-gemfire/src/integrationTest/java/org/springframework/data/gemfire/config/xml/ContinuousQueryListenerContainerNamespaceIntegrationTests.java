...
public class ContinuousQueryListenerContainerNamespaceIntegrationTests {
	...
	public void testContainerConfiguration() throws Exception {
		...
		Assertions.assertThat(container).as("The ContinuousQueryListenerContainer was not properly configured").isNotNull();
		Assertions.assertThat(container.isActive()).as("The CQ Listener Container should be active (initialized)").isTrue();
		Assertions.assertThat(container.isAutoStartup()).as("The CQ Listener container should not be configured to auto-start")
			.isFalse();
		Assertions.assertThat(container.isRunning()).as("The CQ Listener Container should not be running").isFalse();
		Assertions.assertThat(container.getPhase()).isEqualTo(4);
		Assertions.assertThat(testErrorHandler).isNotNull();
		Assertions.assertThat(TestUtils.<Object>readField("errorHandler", container)).isSameAs(testErrorHandler);
		Assertions.assertThat(testTaskExecutor).isNotNull();
		Assertions.assertThat(TestUtils.<Object>readField("taskExecutor", container)).isSameAs(testTaskExecutor);

		CqQuery[] queries = gemfireCache.getQueryService().getCqs();

		Assertions.assertThat(queries).isNotNull();
		Assertions.assertThat(queries.length).isEqualTo(3);
		...
		for (CqQuery query : queries) {
			...
			Assertions.assertThat(query.getQueryString()).isEqualTo("SELECT * FROM /test-cq");
			Assertions.assertThat(query.isDurable()).isEqualTo("Q3".equalsIgnoreCase(query.getName()));
			...
			Assertions.assertThat(listener instanceof ContinuousQueryListenerAdapter).isTrue();
			Assertions.assertThat(((ContinuousQueryListenerAdapter) listener).getDelegate() instanceof GemfireMDP).isTrue();

			if ("Q2".equalsIgnoreCase(query.getName())) {
				Assertions.assertThat(TestUtils.<String>readField("defaultListenerMethod", listener)).isEqualTo("handleQuery");
			}
		}
