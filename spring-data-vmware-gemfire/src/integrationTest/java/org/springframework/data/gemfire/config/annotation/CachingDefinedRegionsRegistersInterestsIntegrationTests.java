...
public class CachingDefinedRegionsRegistersInterestsIntegrationTests extends IntegrationTestsSupport {
	...
	public void cachesRegisterInterestInTestKey() {
		...
		Assertions.assertThat(this.cacheOne).isNotNull();
		Assertions.assertThat(this.cacheOne.getName()).isEqualTo("CacheOne");
		Assertions.assertThat(this.cacheTwo).isNotNull();
		Assertions.assertThat(this.cacheTwo.getName()).isEqualTo("CacheTwo");

		Arrays.asList(this.cacheOne, this.cacheTwo).forEach(cache ->
			Mockito.verify(cache, Mockito.times(1))
				.registerInterest(ArgumentMatchers.eq(testInterest.getKey()), ArgumentMatchers.eq(testInterest.getPolicy()), ArgumentMatchers.eq(testInterest.isDurable()),
					ArgumentMatchers.eq(testInterest.isReceiveValues())));
	}
