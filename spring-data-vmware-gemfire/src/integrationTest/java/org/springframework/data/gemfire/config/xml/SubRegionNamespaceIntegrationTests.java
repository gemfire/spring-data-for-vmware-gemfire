...
public class SubRegionNamespaceIntegrationTests extends IntegrationTestsSupport {
    ...
    public void testNestedRegionsCreated() {
        ...
        Assertions.assertThat(cache.getRegion("parent")).isNotNull();
        Assertions.assertThat(cache.getRegion("/parent/child")).isNotNull();
        Assertions.assertThat(cache.getRegion("/parent/child/grandchild")).isNotNull();
    }
	...
	public void testNestedReplicatedRegions() {
		...
		Assertions.assertThat(child.getName()).isEqualTo("child");
		Assertions.assertThat(child.getFullPath()).isEqualTo("/parent/child");
		Assertions.assertThat(parent.getSubregion("child")).isSameAs(child);
		Assertions.assertThat(grandchild).isNotNull();
		Assertions.assertThat(grandchild.getName()).isEqualTo("grandchild");
		Assertions.assertThat(grandchild.getFullPath()).isEqualTo("/parent/child/grandchild");
		Assertions.assertThat(child.getSubregion("grandchild")).isSameAs(grandchild);
	}
	...
	public void testNestedRegionsWithSiblings() {
		...
		Assertions.assertThat(child1.getFullPath()).isEqualTo("/parentWithSiblings/child1");

		Region child2 = requireApplicationContext().getBean("/parentWithSiblings/child2", Region.class);

		Assertions.assertThat(child2.getFullPath()).isEqualTo("/parentWithSiblings/child2");
		Assertions.assertThat(parent.getSubregion("child1")).isSameAs(child1);
		Assertions.assertThat(parent.getSubregion("child2")).isSameAs(child2);

		Region grandchild1 = requireApplicationContext().getBean("/parentWithSiblings/child1/grandChild11", Region.class);

		Assertions.assertThat(grandchild1.getFullPath()).isEqualTo("/parentWithSiblings/child1/grandChild11");
	}
	...
	public void testComplexNestedRegions() throws Exception {
		...
		Assertions.assertThat(grandchild11FactoryBean).isNotNull();

		CacheLoader expectedCacheLoader = TestUtils.readField("cacheLoader", grandchild11FactoryBean);

		Assertions.assertThat(expectedCacheLoader).isNotNull();

		CacheLoader actualCacheLoader = grandchild11.getAttributes().getCacheLoader();

		Assertions.assertThat(actualCacheLoader).isSameAs(expectedCacheLoader);
	}
}
