...
public class EnableIndexingConfigurationUnitTests extends SpringApplicationContextIntegrationTestsSupport {
	...
	static class GemFireConfiguration {
		...
		@Bean
		ClientCache gemfireCache() throws Exception {
			return mockQueryService(mockRegionFactory(mock(ClientCache.class, "MockGemFireCache")));
		}

		@SuppressWarnings("deprecation")
		ClientCache mockQueryService(ClientCache mockCache) throws Exception {

			QueryService mockQueryService = mock(QueryService.class);

			when(mockCache.getQueryService()).thenReturn(mockQueryService);
			when(mockCache.getQueryService(any(String.class))).thenReturn(mockQueryService);
			when(mockCache.getQueryService(anyString())).thenReturn(mockQueryService);
			when(mockCache.getLocalQueryService()).thenReturn(mockQueryService);
		...
		ClientCache mockRegionFactory(ClientCache mockCache) {

			ClientRegionFactory mockRegionFactory = mock(ClientRegionFactory.class);

			when(mockCache.createClientRegionFactory(any(String.class))).thenReturn(mockRegionFactory);
			when(mockCache.createClientRegionFactory(any(ClientRegionShortcut.class))).thenReturn(mockRegionFactory);
			when(mockCache.createClientRegionFactory(anyString())).thenReturn(mockRegionFactory);
